...
public class ConstantFactory implements IConstantFactory {
    ...
    private BigScreenAdminMapper bigScreenAdminMapper = SpringContextHolder.getBean(BigScreenAdminMapper.class);
    ...
    @Override
    public String getBigScreenNameByCode(final String code) {
        ...
        if (ToolUtil.isEmpty(code)) {
            ...
            BigScrennAdmin param = new BigScrennAdmin();
            param.setCode(code);
            QueryWrapper<BigScrennAdmin> queryWrapper = new QueryWrapper<>(param);
            BigScrennAdmin menu = bigScreenAdminMapper.selectOne(queryWrapper);
            if (menu == null) {
                return "";
            } else {
    ...
    @Override
    public Long getBigScreenIdByCode(String code) {
        if (ToolUtil.isEmpty(code)) {
            return 0L;
        } else if (code.equals("0")) {
            return 0L;
        } else {
            BigScrennAdmin menu = new BigScrennAdmin();
            menu.setCode(code);
            QueryWrapper<BigScrennAdmin> queryWrapper = new QueryWrapper<>(menu);
            BigScrennAdmin tempMenu = this.bigScreenAdminMapper.selectOne(queryWrapper);
            return tempMenu.getBigScreenId();
        }
    }
